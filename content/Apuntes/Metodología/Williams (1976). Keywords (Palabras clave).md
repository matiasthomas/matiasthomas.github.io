...
timeline
    section Antigüedad
	    griego : κριτής (n) : juicio
	    latín : criticus (adj) : decisivo
    section Inglés
	    s. XVI : criticism : descubrir errores
	    s. XVII : criticism : descubrir errores : juzgar literatura
...
```mermaid
flowchart TD
A("`**colō, colere, coluī, cultum**`")
A --- B("`_habitar_`")
B ---> C("`**colonus**`") --> D("`**colonia**`")
A --- E("`_cultivar_`")
E --> K("`**cultivare**
_cultivar \[la tierra]_`")
E ---> F("`**cultura animi**
(Cicerón)`")
K -.-> F
F --> G("`*cultura de...
...
A -->|_a través del alemán_| B("**_(an)aesthetic_**
en relación con el arte")
A -->|"_a través del latín_"| C("**_(an)aesthesic_**
...
B --> D
C --> D
D -->|"(i)"| E("`**Experiencia PASADA**: conocimiento pasado, «lecciones»`")
D -->|"(ii)"| F("`**Experiencia PRESENTE**: tipo especial de conciencia activa`")
```
...
_orden_`") 
A --> B("`**ordinarius** 
_perteneciente al orden, a la autoridad de oficiales_`")
B --> C("`**ordination**
_ordenación_`")
B --> D("`**ordinance**
_ordenanza_`")
C~~~E@{ shape: braces, label: "relacionado con oficiales designados" }
D~~~E
```
...
flowchart LR
A["`**Lo ordinario** (i)
(lo que se hace siguiendo las reglas o la autoridad)`"]
A==>B["`**Lo ordinario** (ii)
(lo que se hace siguiendo costumbres)`"]
B==>C[["`**Lo ordinario** (iii)
(lo que hace el populacho)`"]]
```

A partir del s. XVIII, la relación entre lo «ordinario» y lo «acostumbrado» genera un tono de superioridad, una división cada vez más evidente entre la «gente ordinaria» (inculta, ignorante) y la «gente **sensible**», «decente», «normal», etcétera. (Ver **POPULAR**).
